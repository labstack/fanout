...
import { EmptyState, RefreshButton } from "./components";
...
function LogsApp() {
  ...
  return <main className={`app ${host?.theme === "dark" ? "dark" : ""}`}>
    <header className="header"><div><div className="eyebrow">Application activity</div><h1 className="title">Logs</h1>{result && <p className="summary">{result.data.entries.length} entries in this time range</p>}</div><RefreshButton onClick={() => callTool("search_logs")} disabled={!app} /></header>
    ...
    {result && result.data.entries.length === 0 && <><EmptyState icon={<ListMagnifyingGlass size={18} weight="duotone" />} title="No logs matched">Try a wider time window, a different service, or a less restrictive search.</EmptyState><footer className="meta"><span>{windowLabel(result.provenance.window)}</span><span>No entries found</span></footer></>}

...
function GraphView({ data, selected, dark, onSelect }: { data: Topology; selected: string | null; dark: boolean; onSelect: (id: string) => void }) {
  const option = useMemo(() => {
    ...
    return { tooltip: { backgroundColor: colors.surface, borderColor: colors.border, textStyle: { color: colors.text, fontSize: typeScale.micro } }, series: [{ type: "graph", layout: "force", roam: true, draggable: true, force: { repulsion: 220, edgeLength: [80, 150], gravity: .08 }, label: { show: true, position: "bottom", color: colors.text, fontSize: typeScale.micro }, edgeSymbol: ["none", "arrow"], edgeSymbolSize: 6, data: data.nodes.map((node) => ({ id: node.service, name: node.service, value: node.spans, symbol: healthSymbol(node.health), symbolSize: Math.min(46, healthSymbolScale(node.health) * (24 + Math.log10(Math.max(node.spans, 1)) * 5)), itemStyle: { color: colors.surface, borderColor: healthHex(node.health, dark), borderWidth: selected === node.service ? 5 : 3, borderType: healthBorderType(node.health), opacity: selected && selected !== node.service ? .45 : 1 } })), links: data.edges.map((edge) => { const failing = edge.error_rate >= .05; const dimmed = selected && edge.caller !== selected && edge.callee !== selected; return { source: edge.caller, target: edge.callee, value: edge.calls, lineStyle: { width: failing ? 4 : Math.min(3.5, 1 + Math.log10(Math.max(edge.calls, 1))), color: failing ? status.bad : colors.muted, opacity: dimmed ? .1 : failing ? .95 : .3, curveness: .08 } }; }), emphasis: { focus: "adjacency", lineStyle: { opacity: .85 } } }] };
  }, [dark, data, selected]);
...
function FlowView({ data, dark, onSelect }: { data: Topology; dark: boolean; onSelect: (id: string) => void }) {
  ...
  const option = useMemo(() => {
    const colors = chartTheme(dark);
    return { tooltip: { trigger: "item", backgroundColor: colors.surface, borderColor: colors.border, textStyle: { color: colors.text, fontSize: typeScale.micro } }, series: [{ type: "sankey", left: 20, right: 30, top: 20, bottom: 20, nodeWidth: 14, nodeGap: 12, draggable: true, emphasis: { focus: "adjacency" }, label: { color: colors.text, fontSize: typeScale.micro }, lineStyle: { color: "gradient", opacity: .28, curveness: .55 }, data: data.nodes.map((node) => ({ name: node.service, itemStyle: { color: healthHex(node.health, dark), borderColor: colors.surface, borderWidth: 2 } })), links: links.map((edge) => ({ source: edge.caller, target: edge.callee, value: Math.max(edge.calls, 1) })) }] };
...
function MatrixView({ data, dark }: { data: Topology; dark: boolean }) {
  ...
  const option = useMemo(() => {
    const colors = chartTheme(dark);
    const status = statusHex(dark);
    return { grid: { left: 100, right: 25, top: 15, bottom: 80 }, tooltip: { backgroundColor: colors.surface, borderColor: colors.border, textStyle: { color: colors.text, fontSize: typeScale.micro }, formatter: (params: { data: [number, number, number, number, number] }) => `${names[params.data[1]]} → ${names[params.data[0]]}<br/>${integer.format(params.data[3])} calls · ${duration(params.data[4])}<br/>${percent(params.data[2])} errors` }, xAxis: { type: "category", data: names, splitArea: { show: true }, axisLabel: { color: colors.muted, rotate: 35, fontSize: typeScale.micro }, axisLine: { lineStyle: { color: colors.border } } }, yAxis: { type: "category", data: names, splitArea: { show: true }, axisLabel: { color: colors.text, fontSize: typeScale.micro }, axisLine: { lineStyle: { color: colors.border } } }, visualMap: { min: 0, max, calculable: true, orient: "horizontal", left: "center", bottom: 8, textStyle: { color: colors.muted, fontSize: typeScale.micro }, inRange: { color: [colors.grid, status.warn, status.bad] } }, series: [{ type: "heatmap", data: data.edges.map((edge) => [names.indexOf(edge.callee), names.indexOf(edge.caller), edge.error_rate, edge.calls, edge.average_ms]) }] };
  }, [dark, data, max, names]);
...
/* An ungraded service is not a healthy one: falling through to green claimed
   health the data never established. */
function healthHex(health: string, dark: boolean) { const status = statusHex(dark); return health === "unhealthy" ? status.bad : health === "degraded" ? status.warn : health === "unknown" ? chartTheme(dark).muted : status.ok; }

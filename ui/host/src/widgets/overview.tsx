...
import { chartTheme, healthColor, statusHex } from "../../../chart";
  ...
  return <Stack gap="sm">
    <SimpleGrid cols={2} spacing="sm">
      ...
      <Metric label="Error rate" value={data && !empty ? percent(data.error_rate) : "—"} color={data && data.error_rate >= 0.01 ? "bad" : undefined} hint={data && !empty ? `${integer.format(data.total_spans)} operations` : undefined}>
        {errorTrend.length > 1 && <Sparkline values={errorTrend} color={status.bad} baseline={chartTheme(dark).border} label="Error rate trend" format={(value) => `${value.toFixed(2)}%`} />}
      </Metric>
    </SimpleGrid>
    {data && !empty && <Box>

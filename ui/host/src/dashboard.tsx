...
import { ArrowClockwise, ArrowUpRight, Plus, X } from "@phosphor-icons/react";
...
export default function Dashboard({ onOpenChat }: { onOpenChat: (prompt?: string) => void }) {
  ...
  return <main className="dashboard">
    <div className="dashboard-heading"><div><p className="eyebrow">WORKSPACE</p><h1>System overview</h1><p>Monitor the signals that matter and open a conversation when you need context.</p></div><button className="primary" onClick={() => void queryClient.invalidateQueries()}><ArrowClockwise size={16} weight="bold" aria-hidden="true" className={overview.isFetching || topology.isFetching ? "button-icon spinning" : "button-icon"} />{overview.isFetching || topology.isFetching ? "Refreshing" : "Refresh"}</button></div>
    <div className="dashboard-toolbar"><label>Window<select value={state.filters.window} onChange={(e) => update({ ...state, filters: { ...state.filters, window: e.target.value } })}><option value="15m">15 minutes</option><option value="1h">1 hour</option><option value="6h">6 hours</option><option value="24h">24 hours</option></select></label><label>Namespace<input value={state.filters.namespace} onChange={(e) => setState({ ...state, filters: { ...state.filters, namespace: e.target.value } })} onBlur={() => update(state)} placeholder="All namespaces" /></label><span className="dashboard-save"><i />{saving ? "Saving" : "Saved"}</span><div className="widget-actions"><button className="ghost" onClick={() => add("overview")}><Plus size={15} weight="bold" aria-hidden="true" />Add view</button><button className="ghost quiet" onClick={() => onOpenChat()}>Ask Fanout<ArrowUpRight size={15} weight="bold" aria-hidden="true" /></button></div></div>
    <Grid className="dashboard-grid" layouts={layouts} breakpoints={{ lg: 1100, md: 800, sm: 600, xs: 420, xxs: 0 }} cols={{ lg: 12, md: 10, sm: 6, xs: 2, xxs: 1 }} rowHeight={76} margin={[16, 16]} compactType="vertical" draggableCancel="button,input,select,textarea,a" onBreakpointChange={setBreakpoint} onDragStop={(layout: any) => { if (breakpoint === "lg") update({ ...state, layout }); }} onResizeStop={(layout: any) => { if (breakpoint === "lg") update({ ...state, layout }); }}>

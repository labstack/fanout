  ...
  return <main className="dashboard">
    ...
    <div role="group" aria-label="Dashboard controls" className="mb-5 flex flex-wrap items-end gap-3.5 rounded-2xl border border-line bg-panel/95 p-3.5">
      ...
      <div className="flex items-center gap-3">
        <Select quiet label="Add view" value="" placeholder="Add view" onValueChange={(type) => add(type as WidgetType)} options={Object.entries(widgetTitles).map(([value, label]) => ({ value, label }))} icon={<Plus size={15} weight="bold" aria-hidden="true" />} className="w-[124px] gap-2 px-3 text-text-soft hover:bg-transparent hover:text-text focus-visible:!outline-offset-1" />
        <button className="inline-flex h-10 items-center gap-2 rounded-lg px-3 text-xs font-semibold text-muted transition hover:text-text focus-visible:outline-2 focus-visible:!outline-offset-1 focus-visible:outline-accent" onClick={() => onOpenChat()}>Ask Fanout<ArrowUpRight size={15} weight="bold" aria-hidden="true" /></button>
      </div>
    </div>

...
LOCATION="${3:-fsn1}"
OUTPUT_DIR="${4:-$ROOT/openspec/changes/evaluate-single-binary-data-plane/evidence/linux-screen-$(date -u +%Y%m%dT%H%M%SZ)}"
...
EXCLUSIONS='export_latency_ms.mean_ms=sub-millisecond-screen-jitter-use-quantiles,query_latency_by_operation.overview.mean_ms=periodic-stall-sensitive-use-quantiles,query_latency_by_operation.topology.mean_ms=periodic-stall-sensitive-use-quantiles,query_latency_by_operation.performance.mean_ms=periodic-stall-sensitive-use-quantiles,query_latency_by_operation.trace.mean_ms=periodic-stall-sensitive-use-quantiles,query_latency_by_operation.logs.mean_ms=periodic-stall-sensitive-use-quantiles,server.avg_flush_ms=legacy-low-sample-summary-use-throughput-and-resource-guardrails,server.avg_rollup_ms=legacy-low-sample-summary-use-query-and-resource-guardrails,server.gc_pause_seconds_delta=near-zero-screen-total-use-allocation-and-cpu,server.lake_partitions=compaction-phase-gauge-use-growth-rate,server.lake_partitions_delta=compaction-phase-gauge-use-growth-rate'
FANOUT_PID=0
stop_fanout() {
...
run_suite() {
  ...
  if [ "$server_ready" -ne 1 ]; then
    echo "Fanout readiness failed for $candidate" >&2
    tail -50 "$log_file" >&2
    stop_fanout
    return 1
  fi

  /root/bin/bench orchestrate \
    -output-dir "$suite_dir" \
    -screening \
    -stage "$stage" \
    -candidate "$candidate" \
    -guardrail-exclude "$EXCLUSIONS" \
    ...
    -services 30 \
    -namespaces 1 \
    -attr-cardinality 100 \
    -error-rate 0.05 \
    -messaging-ratio 0.2 \
    -logs=true \
    -metrics=true \
    -metrics-url http://127.0.0.1:7520/-/metrics \
    -query-url http://127.0.0.1:7520 \
    -query-workers 2 \
    -query-rate 5 \
    -max-query-p95-ms 1500 \
    -seed 42 \
    -dataset-id linux-overhead-cpx32-v1 \
    -fanout-version "$fanout_version" \
    -fanout-source-sha256 "$SOURCE_SHA" \
    -measurement-instrumentation "$measurement_mode" \
    -memory-limit-bytes "$MEMORY_LIMIT_BYTES" \
    -duckdb-max-conns 4 \
    -duckdb-memory 6GB \
    -duckdb-threads 4 \
    -flush-seconds 15 \
    -flush-batch-size 50000 \
    -rollup-every-seconds 60 \
    -merge-every-seconds 60 \
    -maintenance-every-seconds 3600 \
    -retention-days 30
  suite_status=$?
  stop_fanout
  return "$suite_status"
...
status=0
run_suite /root/bin/fanout-control "measurement-disabled-${SOURCE_SHA}" control linux-overhead-control "$EVIDENCE/control-suite" /root/control-data /root/control.log disabled || status=1
run_suite /root/bin/fanout-instrumented "measurement-enabled-${SOURCE_SHA}" instrumented linux-overhead-instrumented "$EVIDENCE/instrumented-suite" /root/instrumented-data /root/instrumented.log enabled || status=1

/root/bin/bench analyze \
  -baseline "$EVIDENCE/control-suite/suite.json" \
  -candidate "$EVIDENCE/instrumented-suite/suite.json" \
  -output "$EVIDENCE/analysis.json" || status=1

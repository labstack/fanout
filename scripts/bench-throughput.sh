...
# Decoupled compaction cadences (match the prod design):
#   MERGE_SECONDS — cheap frequent merge that bounds the queryable file count
#   MAINT_SECONDS — hourly heavy pass (retention + expire + cleanup), carries the
#                   deletions. At 3600s it won't fire in a ~40-min run, which is

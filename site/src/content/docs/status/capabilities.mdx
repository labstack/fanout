...
pre-release product should do. Most pages here are `preview` because their
user-facing contracts are still being refined. The marker describes the
current stability of a capability; it does not imply an unlisted compatibility
surface.
...
|---|---|---|
| OTLP ingest over gRPC and HTTP | shipped | Both transports, one port and one ingest token. |
| Parquet storage, trace indexes, and DuckDB analytics | shipped | Authoritative local Parquet; rebuildable DuckDB query state. |
...
| Publication benchmark | preview | A qualified candidate is visible on [Performance](/explanation/performance); repeatable public artifacts are still required. |

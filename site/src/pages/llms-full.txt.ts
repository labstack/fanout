...
// agent reading top to bottom should meet what Fanout is before it meets a
// settings table.
//
// The landing page groups as "index" rather than under a directory, so it is
// listed explicitly — otherwise it is dropped from a document headed "complete
// documentation".
const ROOT = "index";
...
export const GET: APIRoute = async () => {
  ...
  const parts = [
    "# Fanout — complete documentation",
    "",
    "One self-hosted binary for OpenTelemetry: OTLP ingest, Parquet storage,",
    "DuckDB queries, alerts, a chat investigator, MCP tools and the browser",
    "client, in a single Go process. Generated from the documentation source.",
    "Sections marked PREVIEW run today but may change shape without a migration",
    "path; sections marked PLANNED do not execute yet.",
    "",
  ];
  ...
  for (const doc of ranked) {
    ...
    if (doc.data.status !== "shipped") {
      parts.push(`Status: ${doc.data.status.toUpperCase()}`);
    }

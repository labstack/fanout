...
The MCP endpoint requires a user API key. Generate one with `POST /api/auth/api-key` from an authenticated session, then pass it as `Authorization: Bearer fo_<token>` if your transport supports custom headers. The ingest token is for OTLP only and is not accepted by `/mcp`.
...
Ingest and user API keys are deliberately separate: a collector credential cannot query telemetry or open an MCP session.

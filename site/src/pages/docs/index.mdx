...
- **Interactive results** are five domain MCP Apps: health, topology, performance, trace, and logs. They are resources served by `/mcp` and hosted with the official MCP `AppBridge`, not Fanout-specific dashboard blocks.
...
The shell is intentionally small. Product-specific visual components should be added as portable MCP Apps so the same tool can render in Fanout and other MCP App hosts.
...
Both tools are read-only, window- and namespace-bounded, return structured

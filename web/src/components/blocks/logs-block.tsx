...
function EntryRow({ entry, onAction }: { entry: LogEntry; onAction?: (prompt: string) => void }) {
  ...
  return (
    <div className={`group flex items-start gap-3 px-3 py-1.5 hover:bg-white/[0.03] border-l-2 ${
      ...
      {entry.traceId && (
        <button
          className="shrink-0 opacity-0 group-hover:opacity-100 transition-opacity text-[10px] font-mono text-blue-400/60 hover:text-blue-400 hover:underline pt-0.5"

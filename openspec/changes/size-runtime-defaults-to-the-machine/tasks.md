...
- [x] 2.3 Leave the value empty when detection fails, and warn, so an unknown machine keeps DuckDB's behavior instead of being capped at a guess. Evidence: `TestResolveDuckDBMemoryDeclinesWhenMemoryIsUnknown`, and the startup warning observed on macOS where no cgroup or `/proc/meminfo` exists.
- [x] 2.4 Cover detection and resolution with tests, including the unlimited sentinel and the undetectable case. Evidence: `internal/env/sizing_test.go`.

...
- [ ] 1.6 Screen measurement overhead on the isolated Linux decision host, comparing link-time disabled and enabled binaries built from one recorded source archive. Local code validation passes. Darwin screening is rejected as acceptance evidence: an identical-binary null run swung CPU 19 percent and query p95 98 ms to 1055 ms, so that protocol cannot resolve a five-percent question. Run the same-binary null comparison first — if its spread is not small, fix the protocol before screening anything. Evidence: `scripts/bench-overhead-hetzner.sh`, `evidence/measurement-screening.md`.

...
func (s *UserStore) GetByID(id string) (User, error) {
	row := s.db.QueryRow(
		`SELECT id, email, name, role, active, logged_in_at, created_at, updated_at FROM users WHERE id = ?`, id,
	)
...
func (s *UserStore) GetByEmail(email string) (User, error) {
	row := s.db.QueryRow(
		`SELECT id, email, name, role, active, logged_in_at, created_at, updated_at FROM users WHERE email = ?`, email,
	)
	return scanUserRow(row)
}
...
func (s *UserStore) List() ([]User, error) {
	...
	for rows.Next() {
		...
		var active int
		if err := rows.Scan(&u.ID, &u.Email, &name, &u.Role, &active, &loggedIn, &u.CreatedAt, &u.UpdatedAt); err != nil {

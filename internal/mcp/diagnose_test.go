...
func TestRenderDiagnose(t *testing.T) {
	diag := &DiagnoseOut{
		Service: "api-gateway",
		Status:  "healthy",
		...
		TopErrors: []ErrorDetail{
			{Message: "connection timeout", Count: 50, ExampleTrace: "trace-123"},
		},
	...
	output := renderDiagnose(diag)

	if output.ASCII == "" {
		t.Error("renderDiagnose() should produce ASCII output")
	}
	if output.HTML == "" {
		t.Error("renderDiagnose() should produce HTML output")
	}
}
...
func TestRenderDiagnose_Empty(t *testing.T) {
	...
	if output.ASCII == "" {
		t.Error("renderDiagnose() should produce ASCII output for empty data")
	}
}

func TestRenderDiagnose_Unhealthy(t *testing.T) {
	diag := &DiagnoseOut{
		...
		TopErrors: []ErrorDetail{
			{Message: "database connection failed", Count: 25, ExampleTrace: "trace-abc"},
			{Message: "timeout waiting for response", Count: 15, ExampleTrace: "trace-def"},
		},
		SlowOperations: []SlowOperation{},
		Dependencies: []Dependency{
	...
	if output.ASCII == "" || output.HTML == "" {
		t.Error("renderDiagnose() should produce output for unhealthy service")
	}
}

...
WITH producers AS (
  SELECT
    date_trunc('minute', epoch_ms(CAST("name=start_unix_nano"/1000000 AS BIGINT))) AS bucket,
    "name=service_name" AS service,
    json_extract_string(CAST("name=attributes_json" AS VARCHAR), '$.messaging.destination.name') AS destination,
    json_extract_string(CAST("name=attributes_json" AS VARCHAR), '$.messaging.system') AS msg_system
  FROM read_parquet(['%s/spans/tenant=*/namespace=*/year=*/month=*/day=*/hour=*/*.parquet'], union_by_name=true, hive_partitioning=true)
  WHERE "name=kind" = 'SPAN_KIND_PRODUCER'
    AND json_extract_string(CAST("name=attributes_json" AS VARCHAR), '$.messaging.destination.name') IS NOT NULL
),
consumers AS (
  SELECT
    date_trunc('minute', epoch_ms(CAST("name=start_unix_nano"/1000000 AS BIGINT))) AS bucket,

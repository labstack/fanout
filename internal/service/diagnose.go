...
`, s.cfg.LakeDir, window, escapeLike(svc))
...
WITH downstream AS (
  ...
  FROM read_parquet('%s/spans/year=*/month=*/day=*/hour=*/part-*.parquet') parent
  JOIN read_parquet('%s/spans/year=*/month=*/day=*/hour=*/part-*.parquet') child
...
ORDER BY calls DESC
LIMIT 10;
`, s.cfg.LakeDir, s.cfg.LakeDir, window, escapeLike(svc), escapeLike(svc))

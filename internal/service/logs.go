...
SELECT
  %s,
  count(*) AS count,
  list(body ORDER BY random() LIMIT 3) AS sample_bodies,
  list(trace_id ORDER BY random() LIMIT 3) FILTER (WHERE trace_id IS NOT NULL AND trace_id != '') AS sample_trace_ids
FROM logs
WHERE time >= now() - INTERVAL %d MINUTE
  %s
GROUP BY %s

...
- Configure is a `Popover` writing `widget.config`: `service` (select fed by the overview's service list) for overview, performance and topology; `severity` (select) and `search` (text) for logs; `trace_id` (text) for trace. Saving happens on popover close.

...
type config struct {
	...
	seed          uint64
	// Reproducibility metadata. These values are report-only and never change
	// Fanout configuration or expose credentials.
	stage               string
	candidate           string
	primaryTarget       string
	guardrailExclusions string
	runOrdinal          int
	datasetID           string
	fanoutVersion       string
	fanoutSourceSHA256  string
	...
	flushSeconds        int
	flushBatchSize      int
	rollupEverySeconds  int
	mergeEverySeconds   int
	maintenanceSeconds  int
	retentionDays       int
	rollupSkipToLatest  bool
	screening           bool
}
...
func main() {
	...
	flag.Usage = func() {
		fmt.Fprintf(flag.CommandLine.Output(), "Usage:\n  bench [trial flags]\n  bench orchestrate [suite flags] -- [trial flags]\n  bench analyze -baseline BASE/suite.json -candidate CANDIDATE/suite.json -output analysis.json\n\nTrial flags:\n")
		flag.PrintDefaults()
	}
	...
	flag.StringVar(&cfg.stage, "stage", "unspecified", "evidence stage recorded in the report manifest")
	flag.StringVar(&cfg.candidate, "candidate", "control", "candidate identifier recorded in the report manifest")
	flag.StringVar(&cfg.primaryTarget, "primary-target", "", "predeclared primary target metric")
	flag.StringVar(&cfg.guardrailExclusions, "guardrail-exclude", "", "comma-separated metric=rationale guardrail exclusions")
	flag.IntVar(&cfg.runOrdinal, "run-ordinal", 0, "comparable run ordinal (normally 1..3)")
	flag.StringVar(&cfg.datasetID, "dataset-id", "", "logical dataset identifier; defaults to the synthetic workload hash")
	flag.StringVar(&cfg.fanoutVersion, "fanout-version", "unknown", "Fanout build/version identifier under test")
	flag.StringVar(&cfg.fanoutSourceSHA256, "fanout-source-sha256", "", "SHA-256 of the Fanout source archive under test")
	flag.StringVar(&cfg.measurementMode, "measurement-instrumentation", "unspecified", "data-plane measurement mode: enabled, disabled, or unspecified")
	flag.DurationVar(&cfg.warmupDuration, "warmup-duration", 0, "warm-up duration performed before this measured run (manifest only)")
	flag.Int64Var(&cfg.memoryLimitBytes, "memory-limit-bytes", 0, "decision-host memory limit in bytes (0 = auto-detect when possible)")
	flag.IntVar(&cfg.duckDBMaxConns, "duckdb-max-conns", 0, "Fanout DUCKDB_MAX_CONNS under test (0 = unspecified)")
	flag.StringVar(&cfg.duckDBMemory, "duckdb-memory", "", "Fanout DUCKDB_MEMORY under test (empty = unspecified/default)")
	flag.IntVar(&cfg.duckDBThreads, "duckdb-threads", 0, "Fanout DUCKDB_THREADS under test (0 = unspecified/default)")
	flag.IntVar(&cfg.flushSeconds, "flush-seconds", 0, "Fanout FLUSH_SECONDS under test (0 = unspecified)")
	flag.IntVar(&cfg.flushBatchSize, "flush-batch-size", 0, "Fanout FLUSH_BATCH_SIZE under test (0 = unspecified)")
	flag.IntVar(&cfg.rollupEverySeconds, "rollup-every-seconds", 0, "Fanout ROLLUP_EVERY under test (0 = unspecified)")
	flag.IntVar(&cfg.mergeEverySeconds, "merge-every-seconds", 0, "Fanout DUCKLAKE_MERGE_EVERY_SECONDS under test (0 = disabled or unspecified)")
	flag.IntVar(&cfg.maintenanceSeconds, "maintenance-every-seconds", 0, "Fanout DUCKLAKE_MAINTENANCE_EVERY_SECONDS under test (0 = unspecified)")
	flag.IntVar(&cfg.retentionDays, "retention-days", 0, "Fanout RETENTION_DAYS under test (0 = disabled or unspecified)")
	flag.BoolVar(&cfg.rollupSkipToLatest, "rollup-skip-to-latest", false, "Fanout ROLLUP_SKIP_TO_LATEST under test")
	flag.BoolVar(&cfg.screening, "screening", false, "measurement-overhead screening run (5% guardrail, no optimization target)")
	flag.Parse()
	...
	rep.ZeroToleranceFailures, rep.ThresholdFailures = evaluateReport(cfg, rep, infrastructureFailures)
	rep.Passed = len(rep.ZeroToleranceFailures) == 0 && len(rep.ThresholdFailures) == 0
	printReport(rep)
	if cfg.reportPath != "" {
	...
	if !rep.Passed {
		failures := append(append([]string(nil), rep.ZeroToleranceFailures...), rep.ThresholdFailures...)
...
// A 2.5% geometric step keeps quantile error below the benchmark's 5%
// guardrail while remaining fixed-size and bounded for multi-hour soaks. Keep
// the release SLO as an exact boundary so values at or below it cannot round
// into a failing bucket.
...
func printReport(r report) {
	...
	if r.Passed {
		...
		failures := append(append([]string(nil), r.ZeroToleranceFailures...), r.ThresholdFailures...)
